// ═══════════════════════════════════════════════════════════════
// storageSync.js — CRM Dra. Ilza Ezequiel  v2
// Usa get() HTTP para carga inicial (confiável)
// Usa onValue WebSocket para atualizações em tempo real
// ═══════════════════════════════════════════════════════════════
...
function writeToFirebase(key, value) {
  ...
  _debounceTimers[key] = setTimeout(async () => {
    try {
      ...
      console.log("[CRM Sync] Salvo:", key.replace("crm_","").replace("_v26",""));
    } catch (err) {
...
export async function initFirebaseSync() {
  ...
  try {
    console.log("[CRM Sync] Carregando dados do Firebase via HTTP...");
    ...
    console.log(`[CRM Sync] ✅ ${keysLoaded} registros sincronizados do Firebase`);
